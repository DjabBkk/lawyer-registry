    ...
    <section>
      ...
      {details.length ? (
        <div className="mt-4 overflow-hidden rounded-xl border border-warm-200 bg-white shadow-sm">
          {details.map(({ area, description, priceLabel, services }, index) => {
            const key = String(area.id)
            const isOpen = Boolean(openItems[key])

            return (
              <div key={key} className={index !== details.length - 1 ? 'border-b border-warm-200' : ''}>
                <button
                  type="button"
                  onClick={() => toggle(key)}
                  className="flex w-full items-center gap-3 px-4 py-4 text-left transition-colors hover:bg-cream-50"
                >
                  <BriefcaseBusiness className="h-4 w-4 shrink-0 text-royal-600" />
                  <span className="min-w-0 flex-1">
                    <span className="block truncate font-semibold text-royal-900">{area.name}</span>
                  </span>
                  <span className="text-sm font-semibold text-royal-900">{priceLabel}</span>
                  {isOpen ? (
                    <ChevronUp className="h-4 w-4 shrink-0 text-royal-600" />
                  ) : (
                    <ChevronDown className="h-4 w-4 shrink-0 text-royal-600" />
                  )}
                </button>
                ...
                >
                  <div className="min-h-0 px-4 pb-4">
                    {description && (
                      <p className="mt-1 text-sm leading-relaxed text-royal-700">{description}</p>
                    )}

                    {services.length > 0 && (
                      <div className="mt-4 overflow-hidden rounded-lg border border-warm-200 bg-cream-50/50">
