...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (segments.length === 0) {
    return withCanonical({
      title: applyTemplate('Accounting Services in {country}', { country: country.name }),
      description: applyTemplate(
        'Browse accounting businesses and accounting services in {country}. Filter by location and practice area.',
  ...
  if (segments.length === 1) {
    ...
    const [location, practiceArea, firm] = await Promise.all([
      ...
      getBusinessBySlug(s1, { businessTypes: ACCOUNTING_BUSINESS_TYPES, locale }),
    ])

    if (location) {
      return withCanonical({
        title: applyTemplate('Accounting Services in {city}, {country}', {
          city: location.name,
          country: country.name,
        }),
        description: applyTemplate(
          'Find accounting businesses and accounting services in {city}, {country}.',
          {
            city: location.name,
            country: country.name,
          },
    ...
    if (practiceArea) {
      return withCanonical({
        title: applyTemplate('{practiceArea} Accounting Services in {country}', {
          practiceArea: practiceArea.name,
          country: country.name,
        }),
        description: applyTemplate(
          'Find experienced {practiceArea} accounting services in {country}.',
          {
  ...
  if (segments.length === 2) {
    ...
    if (location) {
      ...
      return withCanonical({
        title: applyTemplate('{practiceArea} Accounting Services in {city} | {country}', {
          practiceArea: pa.name,
          city: location.name,
          country: country.name,
        }),
        description: applyTemplate(
          'Find {practiceArea} accounting services in {city}, {country}.',
          {
    ...
    if (practiceArea) {
      ...
      return withCanonical({
        title: applyTemplate('{service} | {practiceArea} Accounting Services in {country}', {
          service: q,
          practiceArea: practiceArea.name,
          country: country.name,
        }),
        description: applyTemplate(
          'Browse {practiceArea} accounting services in {country} focused on {service}.',
          {
            practiceArea: practiceArea.name?.toLowerCase(),
            country: country.name,
            service: q,
  ...
  if (segments.length === 3) {
    ...
    return withCanonical({
      title: applyTemplate('{service} | {practiceArea} Accounting Services in {city}', {
        service: q,
        practiceArea: practiceArea.name,
        city: location.name,
      }),
      description: applyTemplate(
        'Browse {practiceArea} accounting services in {city}, {country} focused on {service}.',
        {
          practiceArea: practiceArea.name?.toLowerCase(),
          city: location.name,
          country: country.name,
    ...
    return (
      <>
        <DarkHero
          title={applyTemplate('Accounting Services in {country}', { country: country.name })}
          description={applyTemplate(
            'Browse our directory of accounting businesses. Filter by practice area, location, and more.',
          )}
  ...
  if (segments.length === 1) {
    ...
    if (location) {
      ...
      return (
        <>
          <DarkHero
            title={applyTemplate('Accounting Services in {city}', { city: location.name })}
            description={applyTemplate(
              'Browse accounting businesses in {city}, {country}. Filter by practice area and more.',
        ...
        <>
          <DarkHero
            ...
            description={
              ...
              applyTemplate('Find qualified {practiceArea} accounting services across {country}.', {
                practiceArea: practiceArea.name?.toLowerCase(),
                country: country.name,
              })
            }
  ...
  if (segments.length === 2) {
    ...
    if (location) {
      ...
      return (
        <>
          <DarkHero
            title={applyTemplate('{practiceArea} Accounting Services in {city}', {
              practiceArea: practiceArea.name,
              city: location.name,
            })}
            description={applyTemplate(
              'Find qualified {practiceArea} accounting services in {city}, {country}.',
              {
                practiceArea: practiceArea.name?.toLowerCase(),
                city: location.name,
                country: country.name,
